...
plot_scols = c("CN", "PLOT", "LAT", "LON", "ELEV", "MANUAL", "DESIGNCD", "STATECD", "MEASYEAR") #and create a list of the column you want to reduce csv load time
...
tree_scols = c('CN', 'PLT_CN', 'PLOT', 'SPCD', 'TREE', 'DIA', 'HT', "DIAHTCD", "TPA_UNADJ") #and create a list of the column you want to reduce csv load time
...
###########QUERYING##############
#now we want all the trees including within the bounding box with a DIAHTCD = 1 (meaning the tree diameter was measured for DBH)
trees_req_cols = sprintf("trees.PLT_CN, trees.SPCD, trees.DIA, trees.DIA * 2.54 as 'DBH_CM', trees.HT, trees.HT * 0.3048 as 'HT_M', trees.DIA*trees.DIA *  0.005454154 * trees.TPA_UNADJ as 'BA_ACRE', trees.TPA_UNADJ")
plots_req_cols = sprintf("plots_qa.LAT, plots_qa.LON, plots_qa.ELEV, plots_qa.ELEV * 0.3048 as 'ELEV_M' , plots_qa.STATE_NAME")
...
select_trees = sqldf(sprintf("SELECT PLT_CN, SPCD, COUNT(SPCD) as 'TREE_TOTAL_COUNT', AVG(DIA) as 'MEAN_DBH', sqrt(variance(DIA)) as 'STDEV_DBH', SUM(TPA_UNADJ) as 'TOTAL_TPA' from all_trees_bbox WHERE DIA > 0 GROUP BY PLT_CN, SPCD"))
...
class_trees = sqldf(sprintf("SELECT PLT_CN, STATE_NAME, LAT, LON, ELEV, ELEV_M, %s FROM %s GROUP BY PLT_CN",sql_cmd_final,db_name)) #query and create the pivot table
